...
class RMS:
    def __init__(self, root):
        ...
        #===icons=====
        self.logo_dash=ImageTk.PhotoImage(file="Images/logo_p.png.png")
        #===title=====
        title = Label(self.root, text="Student Result Management System",
                      padx=10,compound=LEFT,image=self.logo_dash,
                      font=("goudy Old Style", 20, "bold"),
                      bg="#033054", fg="white")  # Corrected 'Label' and fixed parentheses
        title.place(x=0, y=0, relwidth=1, height=50)  # Fixed 'height' spelling
        #===Menu====
        ...
        btn_view = Button(M_Frame, text="View student Results", font=("goudy old style", 15, "bold"), bg="#0b5377", fg="white",
        ...
        #====content_window====
        self.bg_img=Image.open("Images/bg.png.png")
        ...
        # Clock========
        self.lbl = Label(self.root, text="\nWebCode Clock", font=("Book Antique", 25, "bold"), fg="white",
                         compound=BOTTOM, bg="#081923", bd=0)
        self.lbl.place(x=10, y=180, height=450, width=350)
        self.working()

        # ===footer=====
        footer = Label(self.root, text="MGKVP.Student Result Management System\nContact Us for any Technical Issue:9795XXX354",
    ...
    def logout(self):
        op=messagebox.askyesno("confirm","Do you really want to logout%s",parent=self.root)
        if op==True:
            self.root.destroy()
            import login
    ...
    def exit_(self):
        op = messagebox.askyesno("Confirm", "Do you really want to Exit%s", parent=self.root)
        if op == True:
            self.root.destroy()
